...
class ActiveFabricSource:
    ...
    def start(self):
        ...
        def initiate_thread(thread_obj, name, thread_runner, queue, subscriptions):
            if thread_obj is None or thread_obj.is_alive() is not True:
                thread_obj = Thread(
                    ...
                    args=(self.fabric_task_event, queue, subscriptions),
                )
        ...
        self.lifo_thread = initiate_thread(
            thread_obj=self.lifo_thread,
        ...
        def stop_thread(thread_obj, queue):
            if thread_obj is not None:
                if thread_obj.is_alive() is True:
                    stop_fe = FabricEvent(
                        HsmEvent(signal=signals.STOP_FABRIC_SIGNAL), priority=1
                    )
...
class ActiveObject(HsmWithQueues):
    ...
    def publish(self, event, priority=None):
        ...
        payload = PublishEvent(event=event, priority=priority)
        self.post_lifo(HsmEvent(signal=signals.PUBLISH_META_SIGNAL, payload=payload))
    ...
    def __post_event(self, e, times=None, period=None, deferred=None, queue_type=None):
        ...
        if times == 1 and period is None:
            ...
            posted_event_thread_spec = self.PostedEventThreadSpec(
                event=e,
...
class ThreadSafeAttributes(metaclass=MetaThreadSafeAttributes):
    pass


class ActiveObjectWithAttributes(ThreadSafeAttributes, ActiveObject):
    pass


class FactoryWithAttributes(ThreadSafeAttributes, Factory):
    pass
